...
public:
    ...
    I2C(uint8_t Address, SSD1306ControlBytes ControlBytes, SSD1306Commands Commands, SSD1306Pins Pins, SSD1306Configuration Configuration);

    [[gnu::hot]]  void Draw(const uint8_t Segment, const uint8_t Page, uint8_t Width, uint8_t Offset, uint8_t& Data) const noexcept;
...
private:
    ...
    void                    Mode(const uint8_t Mode, i2c_cmd_handle_t& CommandHandle) const noexcept;
    ...
    [[gnu::hot]]  void      IndexGDDRAM(const uint8_t Segment, const uint8_t Page, uint8_t Offset, uint8_t& Data) const noexcept;
};
